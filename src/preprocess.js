...
export default function preprocess({ path, contents, dirMap, appDir, needsResolve, resolvePath = false, useStrict = true, sourceMap = true, verbose, silent }) {
  ...
  try {
    const { code, updated } = transform(contents, path, { useStrict, sourceMap, resolvePath })
    if (verbose) {
