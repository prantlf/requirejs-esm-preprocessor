...
export default function preprocess({ path, contents, dirMap, appDir, needsResolve, resolvePath = false, sourceMap = true, verbose, silent }) {
  ...
  try {
    const { code, updated } = transform(contents, path, { sourceMap, resolvePath })
    if (verbose) {
